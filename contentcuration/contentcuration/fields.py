...
def uuid_to_hex(uuid_obj):
    ...
    Raises:
        ValueError: If uuid_obj cannot be converted to UUID
    """
    if not uuid_obj:
        return None
    ...
    if len(uuid_str) == 32:
        ...
        try:
            ...
            # Not valid hex, fall through to standard UUID parsing
            pass
    ...
    def pre_save(self, model_instance, add):
        ...
        if not char_value and add:
            ...
            try:
                ...
                # Invalid hex data - log error but don't crash the save
                logger.error(
                    f"Invalid UUID hex value in {model_instance.__class__.__name__}.{self.attname}: "
                    f"{char_value!r}: {e}"
                )
                # Leave shadow column as-is (NULL or previous value)
    ...
    def pre_save(self, model_instance, add):
        ...
        if char_value:
            try:
                ...
                # Invalid hex data - log error but don't crash the save
                logger.error(
                    f"Invalid UUID hex value in {model_instance.__class__.__name__}.{self.name}_id: "
                    f"{char_value!r}: {e}"
                )
                # Leave shadow column as-is (NULL or previous value)
        else:
