...
class Command(BaseCommand):
    ...
    def _check_nulls(self, table_name, char_col, uuid_col):
        ...
        char_column = connection.ops.quote_name(char_col)
        uuid_column = connection.ops.quote_name(uuid_col)

        with connection.cursor() as cursor:
            cursor.execute(f"""
                SELECT COUNT(*)
                FROM {table}
                WHERE {char_column} IS NOT NULL AND {uuid_column} IS NULL
            """)
    ...
    def _check_mismatches(self, table_name, char_col, uuid_col):
        ...
        with connection.cursor() as cursor:
            # Compare hex (no hyphens) vs UUID text (with hyphens removed)
            cursor.execute(f"""
                SELECT COUNT(*)
                FROM {table}
                WHERE {char_column} IS NOT NULL
                AND {uuid_column} IS NOT NULL
                AND {char_column}::text != REPLACE({uuid_column}::text, '-', '')
            """)
            return cursor.fetchone()[0]
    ...
    def _check_fk_integrity(self, table_name, fk_col, parent_table, parent_col):
        ...
        # Quote identifiers to prevent SQL injection
        table = connection.ops.quote_name(table_name)
        fk_column = connection.ops.quote_name(fk_col)
        parent = connection.ops.quote_name(parent_table)
        parent_column = connection.ops.quote_name(parent_col)

        with connection.cursor() as cursor:
            cursor.execute(f"""
                SELECT COUNT(*)
                FROM {table} child
                WHERE child.{fk_column} IS NOT NULL
                AND NOT EXISTS (
                    SELECT 1
                    FROM {parent} parent
                    WHERE parent.{parent_column} = child.{fk_column}
                )
            """)
            return cursor.fetchone()[0]
