    ...
    def _backfill_field(self, table_name, char_col, uuid_col, batch_size, dry_run):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(f"""
                SELECT COUNT(*)
                FROM {table}
                WHERE {uuid_column} IS NULL AND {char_column} IS NOT NULL
            """)
            ...
            while True:
                with transaction.atomic():
                    ...
                    cursor.execute(f"""
                        UPDATE {table}
                        SET {uuid_column} = (
                            {char_column}::text::uuid
                        )
                        WHERE {uuid_column} IS NULL
                        AND {char_column} IS NOT NULL
                        AND ctid IN (
                            SELECT ctid
                            FROM {table}
                            WHERE {uuid_column} IS NULL
            ...
            # Validate - compare hex (no hyphens) vs UUID text (with hyphens removed)
            cursor.execute(f"""
                SELECT COUNT(*)
                FROM {table}
                WHERE {char_column} IS NOT NULL
                AND ({uuid_column} IS NULL OR {char_column}::text != REPLACE({uuid_column}::text, '-', ''))
            """)
